...
public class LearnBoostedRDN {
	...
	public Map<String, Double> previousPredictionOther = null;
	...
	public void learnNextModelPrivInfo(RunBoostedModels caller, SRLInference sampler, ConditionalModelPerPredicate rdn, int numMoreTrees, Map<String,Double> predictionOfOtherModel) {
		previousPredictionOther = predictionOfOtherModel;
		Utils.println("\n% Learn model for: " + targetPredicate);
		long start = (debugLevel > 0 ? System.currentTimeMillis() : 0);
		//if (!cmdArgs.isLearnRegression()) {
		// Call facts and examples the first time.
			...
			if (disableBoosting) {
				...
				if (probDistr.isHasDistribution()) {
					...
					if (eg.isHiddenLiteral()) {
						...
						else if(previousPredictionOther != null && !(cmdArgs.getPrivWt()<0)) {
//							Utils.println("Does the example exist: " + previousPredictionOther.containsKey(eg));
							Utils.println(eg.toString() + "\tPrevious grad:" + prob + "\tUpdate: " + previousPredictionOther.get(eg.toString()) + "\t" + cmdArgs.getIsPrivModel() + "\t" + previousPredictionOther.size());
							if (eg.isOriginalTruthValue()) {
								eg.setOutputValue( (1 - prob) - cmdArgs.getPrivWt() * (prob - previousPredictionOther.get(eg.toString()))  );					
							} else {
								eg.setOutputValue( (0 - prob) - cmdArgs.getPrivWt() * (prob - previousPredictionOther.get(eg.toString())));
							}
						} else {
//							Utils.exit("Should not reach here");
							if (eg.isOriginalTruthValue()) {
